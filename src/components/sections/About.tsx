...
export function About() {
  return (
    <section id="about" className="border-t border-[#e2ddd2] bg-[#fdfcf8]">
      ...
      {/* ── Header ── */}
      <div className="px-8 md:px-16 pt-40 pb-32 border-b border-[#e2ddd2] flex flex-col items-center text-center">
        <motion.p
          {...fadeUp(0)}
        ...
        <motion.a
          ...
          className="flex items-center gap-3 font-['DM_Mono'] font-semibold text-[28px] tracking-[0.06em] text-[#0b3d2e] hover:text-[#145c42] transition-colors duration-200"
        >
      ...
      {/* ── Grid ── */}
      <div className="px-8 md:px-16 pt-24 pb-40 grid grid-cols-2 lg:grid-cols-4 gap-4">
        {posts.map((post, i) => (
          <motion.a
            key={i}
            href={post.link}
            ...
            className="relative aspect-square overflow-hidden group rounded-xl border-2 border-[#e2ddd2]"
          >
            {/* Image */}
            <img
              src={post.image}
            ...
            {/* Default overlay — tag + caption always visible at bottom */}
            <div className="absolute inset-0 bg-gradient-to-t from-[#0b3d2e]/80 via-transparent to-transparent" />
            <div className="absolute bottom-0 left-0 right-0 px-4 py-4">
              <span className="font-['DM_Mono'] font-semibold text-[10px] tracking-[0.16em] uppercase text-[#f0c040] block mb-1">
                {post.tag}
              </span>
              <p className="font-['DM_Mono'] font-semibold text-[13px] text-white leading-snug">
                {post.caption}
              </p>
            </div>

            {/* Hover overlay — LinkedIn icon */}
